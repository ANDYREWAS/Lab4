# 1 "../../OneDrive - Universidad del Valle de Guatemala/UVG/UVG 5... - 2021/Progra micros/main_tmr0_int.s"
# 1 "<built-in>" 1
# 1 "../../OneDrive - Universidad del Valle de Guatemala/UVG/UVG 5... - 2021/Progra micros/main_tmr0_int.s" 2
; Archivo: main_tmr0.s
...
# 36 "../../OneDrive - Universidad del Valle de Guatemala/UVG/UVG 5... - 2021/Progra micros/main_tmr0_int.s" 2
...
PSECT udata_bank0
    valor: DS 1
    banderas: DS 1
    nibbles: DS 2 ;Para guardar los valores en hex (una variable para los dos displays, uno nibbles, dos = nibbles + 1
    display: DS 2 ;Recupera los valores de la tabla
...
OBTENER_NIBBLE:
    Movlw 0x0F ;para obtener los ultimos cuatro bits de la var nibble (los menos significativos, con un AND)
    andwf valor, W
    movwf nibbles

    movlw 0xF0 ;para obtener los bits mas significativos
    andwf valor,w
    movwf nibbles+1 ;el +1 es para usar el otro byte de la variable
    swapf nibbles+1,F ;el swap intercambia los bits mas significativos con los menos significativos, que es lo que necesitamos para poder usar la tabla porque está solo recibe 4 bits, no 8 como teníamos antes


    Return

    SET_DISPLAY:
    movf nibbles,w ;movemos el valor del nibble menos significativo a w
    Call tabla_7seg ;usamos el dato del nibble para llamar su representación en la tabla
    movwf display ;movemos la representación en hex de w a la var display

    movf nibbles+1,w ;movemos el valor del nibble menos significativo a w
    Call tabla_7seg ;usamos el dato del nibble para llamar su representación en la tabla
    movwf display+1
    ...
    mostrar_valor:
    clrf PORTD
    BTFSS banderas,0 ;si la bandera está prendida...
    goto DISPLAY_1 ;nos vamos a la subrutina de config del display
    ...
    movwf PORTC
    bsf PORTD,1
    bsf banderas,0 ;bit menos significativo
    return
    ...
    movwf PORTC
    bsf PORTD,0
    bcf banderas,0 ;bit menos significativo
    return
